...
const visual_state = {
    ...
    normal_red_dim: 49,     // normal dimension in pixels for red icons
...
ClickableOverlay.prototype.onAdd = function() {
    ...
    canvas.addEventListener('click', function(e){
        ...
        var y_coord = e.clientY - canvas_coord.bottom;
...
function set_image(robot) {
    ...
    else if (robot.type == robot_type.Police)
    {
        ...
        //image_source = 'images/truck_blue_mp2.png';
    }
...
function set_icon_normal(robot) {
    ...
    if (robot.affiliation === affiliation.RED) {
        robot.setIcon({
            url: robot.image.src,
            scaledSize: new google.maps.Size(visual_state.normal_red_dim, visual_state.normal_red_dim, 'px', 'px'),
        });
    } else {
        robot.setIcon({
        url: robot.image.src,
...
//  var db = "destroy(" + tId + ")";
// log(db);
    var robot = robot_array[tId];
    log(robot_array.length);

    var image_source;
    
    if (robot.type == robot_type.Village) 
    {
        image_source = 'images/villageX.png';
    }
    else if(robot.type == robot_type.NGO)
    {
        image_source = 'images/ngoX.png';
    }
    else if(robot.type == robot_type.Base)
    {
        image_source = 'images/fwdX.png';
    }
    else if (robot.type === robot_type.SUAV)
    {
        image_source = 'images/small_uavX.png';        
    }
    else if (robot.type == robot_type.HUAV)
    {
        image_source = 'images/heavy_uavX.png';
    }
    else if (robot.type == robot_type.Balloon)
    {
        image_source = 'images/blimp_autonomousX.png';
    }
    else if (robot.type == robot_type.Police)
    {
        image_source = 'images/truck_blue_mpX.png';
    }
    else if (robot.type == robot_type.Truck)
    {
        //Blue, red, neutral?
        if (robot.affiliation === affiliation.RED)
        {
            image_source = 'images/truck_red_terX.png';
        }
        else if (robot.affiliation === affiliation.NEUTRAL)
        {
            image_source = 'images/truck_neutral_civX.png';
        }
        else
        {
            image_source = 'images/truck_unknownX.png';
        }
    }
    else if (robot.type == robot_type.Dismount)
    {
        ...
        if (robot.affiliation === affiliation.RED)
        {
            image_source = 'images/dismount_red_terX.png';
        }
        else if (robot.affiliation === affiliation.NEUTRAL)
        {
            image_source = 'images/dismount_neutralX.png';
        }
        else
        {
            image_source = 'images/dismount_unknownX.png';
        }
    }
    else
        image_source = 'images/undefinedX.png';
