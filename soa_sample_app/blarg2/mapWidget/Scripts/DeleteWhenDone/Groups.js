...
function makeGroup(expObj, numId, idString, tiid, hullColor, affiliation, rype, gId) {
	...
	pgon.setMap(map);
	//iid, lat, lng, iype, iitle, msg
	var ttitle = [temp.rtype, 'Group'].join(' ');
	//iid, lat, lng, affiliation, iype
	...
	//log("icon lat, lng: " + gIc.position.lat + ", " + gIc.position.lng);
	gIc.groupIcon = 1;
	var tGroup = new Group(pgon, tId, gIc);
	groupArray[tId] = tGroup;
	set_visual_state2(gIc, 'normal')
	//log("group Icon value; should be 1: " + gIc.groupIcon);
	//log("group Icon id should be: " + gIc.id);
	//log("robot_array[gIc.id].id = " + robot_array[gIc.id].id);
	//log("robot_array[gIc.id].groupIcon = " + robot_array[gIc.id].groupIcon);
	//log(" makeGroup groupArray length:");
	//log(groupArray.length);
	//var QtPts = "WhamWhamWham!!!";
